...
One benefit of being a thin layer is that it is easy to vibe code implementations in other languages. I have done that for Elixir and funnily enough that will likely be the first one I use in an application. I wanted it to feel like just calling functions but there are a few things that can feel like magic. The first is that each of the top level functions return a `Result` struct and you can pass that as any input to another function and the output value will be extracted and the input IDs will be passed through. This is used to build a DAG on the server to do things like see the structure of your workflow in the UI. Also there is the `let` function, this is used when you want to apply some transformation to the `value` of a result while preserving the flow of input IDs. And you can manually set the context and input IDs so you can persist them in the database and start and stop workflows at any time.
